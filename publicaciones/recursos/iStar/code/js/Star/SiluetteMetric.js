...
function IndiceDum(osDados,klassesArray)
{
	console.log("estos son las clases");
	console.log(klassesArray);
	...
	for(i=0;i<klassesArray.length;i++){
		...
		klassesArray[i].diam=max;
		for(j=i+1;j<klassesArray.length;j++){
			MatrixClusterDistaces[i][j]=Math.sqrt((klassesArray[i].ccx-klassesArray[j].ccx)*(klassesArray[i].ccx-klassesArray[j].ccx)+(klassesArray[i].ccy-klassesArray[j].ccy)*(klassesArray[i].ccy-klassesArray[j].ccy));
		}
	/*	for(j=i+1;j<klassesArray.length;j++){
	   //hallamos la distancia entre los clusters //probamos con la distancia minima entre cada punto de las dimensiones
			var sum=0;
			klassesArray[i].indexs.forEach(function(di){
				klassesArray[j].indexs.forEach(function(dj){
			     		sum+=DistanceMatrixMainProjection[parseInt(di)][parseInt(dj)];
					});
				});
			sum=parseFloat(sum/parseFloat(klassesArray[i].indexs.length+klassesArray[j].indexs.length));
			MatrixClusterDistaces[i][j]=sum;
		}*/
	}
	//hallamos el maximo Diametro
	var maxDiam=0;
	klassesArray.forEach(function(d){if(parseFloat(d.diam)>parseFloat(maxDiam)){maxDiam=d.diam;}})

	console.log("ESTE ES EL DIAMETRO MAXIMO:"+maxDiam);
	...
	for(i=0;i<klassesArray.length;i++){
		for(j=i+1;j<klassesArray.length;j++){
			var value=parseFloat(MatrixClusterDistaces[i][j]/maxDiam);
			if(value<ratio){ratio=value;}
		}
	}
