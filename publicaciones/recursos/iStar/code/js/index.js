    /*-----------------------------------------------------------------------------------------*/
   var radioMainProjection=8;var velocidadAnimacion=100;var tolerancia=10;var FocusControlPointIndex=-1;var tamanioFirstDetail=200;var tamanioSecondDetail=300;var marginNP={top:5,right:0,bottom:55,left:0};var NP_width=$("#NPreservation").width();var NP_height=$("#NPreservation").width()*0.6;NP_width=NP_width-marginNP.left-marginNP.right;NP_height=NP_height-marginNP.top-marginNP.bottom;var ColordeDato="#fff";var strokeData=0.4;var ColordeControl="#000";var strokeControl=0.3;var focusX;var focusY;
    /*---------------------------------------------------------------------------------------*/
    SIZEMAINPROJECTION=$("#maincontainer").width();  var contro=0;//20;        variable para regular el tamaño de la ventana;
  ...
  var SecondDetail=d3.select("#secondProj").append("svg").attr("width",SIZESECONDPROJECTION).attr("height",SIZESECONDPROJECTION).append("g");
  var FirstDetail=div.append("svg").attr("width",tamanioFirstDetail).attr("height",tamanioFirstDetail).append("g");
  var FirstDetail=div.append("svg").attr("width",tamanioFirstDetail).attr("height",tamanioFirstDetail).append("g");
  var NPDIV=d3.select("#NPreservation").append("svg").attr("width",NP_width+marginNP.left+marginNP.right).attr("height",NP_height+marginNP.top+marginNP.bottom).append("g").attr("transform","translate("+marginNP.left+","+marginNP.top+")");
    ...
    function StarCoordinatesRender(){
            ...
            var SCMenorX=MAS_INFINITO;var SCMayorX=MENOS_INFINITO;var SCMenorY=MAS_INFINITO;var SCMayorY=MENOS_INFINITO;
                         ...
                         if(d3.event.button == 0) {
                           ...
                           if (d3.event.ctrlKey) {
                               ...
                               if(parseInt(valorMayor)>=0){
                                   ...
                                   MYCONTROLPOINTS[parseInt(valorMayor)].text="Group_"+aux;
    ...
    function Proyectar(){
       ...
       misGrupos=misGrupos.concat(LISTOFDIMENSIONS,ListOfGroupsOfDimensions);MYCONTROLPOINTS=new Array();MYCONTROLPOINTS=distribucion(1,misGrupos);StarCoordinatesRender();
    }
    ...
    {
       var misGrupos=new Array()
       misGrupos=misGrupos.concat(LISTOFDIMENSIONS,ListOfGroupsOfDimensions);MYCONTROLPOINTS=new Array();MYCONTROLPOINTS=distribucion(1,misGrupos);var VArrayControlPoints=new Array();MYCONTROLPOINTS.forEach(function(d,i){VArrayControlPoints[i]=[parseFloat(d.x),parseFloat(d.y)]});GENERALMATRIX.rows.forEach(function(d,i){var valores=d.valores;var Suma=0;valores.forEach(function(f){Suma=parseFloat(Suma)+parseFloat(f);});var Aux=STARCOORDINATESFor_Point(valores,VArrayControlPoints);osDados.push({"id":d.id,"klasses":d.klass,"valores":valores,"xStar":parseFloat(Aux[0]/Suma),"yStar":parseFloat(Aux[1]/Suma)});});DistanceMatrixMainProjectionFunction(osDados);klasesIndices=GetKlasses(osDados);var xStarCoordinates=d3.scale.linear().domain([-1.2,1.2]).range([tolerancia/2,SIZEMAINPROJECTION-tolerancia/2]);var yStarCoordinates=d3.scale.linear().domain([-1.2,1.2]).range([tolerancia/2,SIZEMAINPROJECTION+parseInt(contro)-tolerancia/2]);var CpScale=d3.scale.linear().domain([1,1]).range([10,10]);STAR.selectAll(".star_points").data(osDados).enter().append("g").append("circle").attr("cx",function(d){return xStarCoordinates(d.xStar);}).attr("cy",function(d){return yStarCoordinates(d.yStar);}).attr("r",radioMainProjection).attr("stroke",ColordeDato).attr("stroke-width",strokeData).attr("fill",function(d){return DataColor(parseInt(d.klasses),1000);}).attr("class","circleData");STAR.selectAll(".head_coordinates").data(MYCONTROLPOINTS).enter().append("circle").attr("stroke",ColordeControl).attr("stroke-width",strokeControl).attr("id",function(d,i){return"cp";}).attr("cx",function(d){return xStarCoordinates(d.x);}).attr("cy",function(d){return xStarCoordinates(d.y);}).attr("r",function(d){return CpScale(d.valores.length);}).attr("fill",function(d,i){return d.color;});
    }
    /*Input:
    ...
    //	}
        ...
        var misGrupos=new Array()
               misGrupos=misGrupos.concat(LISTOFDIMENSIONS,ListOfGroupsOfDimensions); //conjunto de puntos de control inicial
               MYCONTROLPOINTS=new Array();
               MYCONTROLPOINTS=distribucion(1,misGrupos); //Distribucion inicial de los puntos de control
               StarCoordinatesRender();
    }
    ...
    function ChargeDetailTable(){
        var id=["id"];var titulo=id.concat(GENERALMATRIX.attributes,["clase"]);var matrix=new Array();osDados.forEach(function(d,i){if(d.drag==1){var id=[d.id];var myArray=id.concat(GENERALMATRIX.rows[i].toArray(),[d.klasses]);matrix.push(myArray);}})
